    ...
    class LoginHelper
    {
        ...
        public static LoginResult LogIn(Account acc)
        {
            ...
            inputs.Add(GetMouseEvent(
                sx * mix(rct.Left, rct.Right, 0.14f), 
                sy * mix(rct.Top, rct.Bottom, 0.13f), 
...
//
            ...
            return (i == inputs.Count) ? LoginResult.Success : LoginResult.Error;
        }

        static int mix(int a, int b, float v)
        {
            return a + (int)((b - a) * v);
        }
        ...
        static INPUT GetKeyboardEvent(char c, bool down, int t)
        {
            return new INPUT()
            {
                type = 1,
                U = new InputUnion()
                {
                    ki = new KEYBDINPUT()
                    {
                        ...
                        dwFlags = KEYEVENTF.UNICODE | (down ? 0 : KEYEVENTF.KEYUP),
                        time = t,
                        dwExtraInfo = UIntPtr.Zero
                    }
                }
        ...
        static INPUT GetKeyboardEvent(ScanCodeShort key, bool down, int t)
        {
            return new INPUT()
            {
                type = 1,
                U = new InputUnion()
                {
                    ki = new KEYBDINPUT()
                    {
                        wVk = 0,
                        wScan = key,
                        dwFlags = KEYEVENTF.SCANCODE | (down ? 0 : KEYEVENTF.KEYUP),
                        time = t,
                        dwExtraInfo = UIntPtr.Zero
                    }
                }
